...
public class DlpBackend(ILogger<DlpBackend> logger)
{
    ...
    {
        var argument = $"{url} {_metadataFlags}";
        ...
        try
        {
            ...
            var parsedVideos = parseResult
                ...
                .Select(s => JsonSerializer.Deserialize(s, SourceGenerationContext.Default.Song))
    ...
    {
        var arguments = $"{url} {_streamFlags}";
    ...
    private class ThumbnailComparer : Comparer<Thumbnail>
    {
        public override int Compare(Thumbnail? x, Thumbnail? y)
        {
            ...
            if (x.Width is not null && x.Height is not null)
                return (x.Width.Value * x.Height.Value).CompareTo(y!.Width * y.Height);
