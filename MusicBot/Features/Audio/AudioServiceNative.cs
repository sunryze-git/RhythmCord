...
public class AudioServiceNative(ILogger<AudioServiceNative> logger) : IDisposable
{
    private static readonly unsafe avio_alloc_context_read_packet _readPacketDelegate = ReadPacketCallback;
    private static readonly ThreadLocal<byte[]> _callbackBuffer = new(() => new byte[65536]);
    ...
    public void Dispose()
    {
        // dispose of unmanaged resources if any in the future
    }

    internal async Task StartAudioStreamAsync(Stream inStream, OpusEncodeStream outStream, CancellationToken stopToken)
    {
        if (inStream == null) throw new ArgumentNullException(nameof(inStream), "Input stream cannot be null.");

        if (outStream == null) throw new ArgumentNullException(nameof(outStream), "Output stream cannot be null.");

        if (!inStream.CanRead) throw new ArgumentException("Input stream must be readable.", nameof(inStream));
    ...
    private void ConvertToPcm(Stream inStream, Stream outStream, CancellationToken token)
    {
        unsafe
        {
            ...
            try
            {
                ...
                ffmpeg.av_dict_set(&opts, "analyzeduration", "100000", 0);
                ...
                var res = ffmpeg.avformat_open_input(&formatCtx, null, null,
                    &opts); // common return value for FFmpeg functions
                if (res != 0)
                {
                    formatCtx = null;
                ...
                if (audioStreamIndex == -1) throw new ApplicationException("The given content has no audio track.");
                ...
                logger.LogInformation("""
                                      Incoming audio stream:
                                           Codec Name: {AvcodecGetName}
                                           Codec ID: {AvCodecId}
                                           Channel Count: {NbChannels}
                                           Sample Rate: {I}
                                           Sample Format: {I1}
                                           Duration: {CurrentSongLength:g}
                                      """, ffmpeg.avcodec_get_name(codecPar->codec_id), codecPar->codec_id,
                    codecPar->ch_layout.nb_channels, codecPar->sample_rate, codecPar->format, CurrentSongLength);
                ...
                res = ffmpeg.swr_alloc_set_opts2(
                    ...
                    0,
                    null);
                if (res < 0) throw new ApplicationException("Failed to allocate resample context.");
                ...
                while (true)
                {
                    ...
                    while (true)
                    {
                        ...
                        {
                            logger.LogDebug("No samples converted, skipping to next packet.");
                            continue;
                        }
            ...
            finally
            {
                ...
                logger.LogInformation("Audio stream processing completed and resources cleaned up.");
            }
        }
    }
    ...
    private static unsafe string GetFFmpegErrorString(int averror)
    {
        ...
        if (ffmpeg.av_strerror(averror, buffer, 256) < 0) return $"Unknown error code: {averror}";
    ...
    private static unsafe int ReadPacketCallback(void* opaque, byte* buf, int bufSize)
    {
        try
        {
            ...
            var buffer = _callbackBuffer.Value;
            if (buffer?.Length < bufSize)
            {
                buffer = new byte[Math.Max(bufSize, 65536)];
                _callbackBuffer.Value = buffer;
            }
        ...
        {
            Console.Error.WriteLine($"[ERROR] ReadPacketCallback: {ex.Message}");
            return ffmpeg.AVERROR(ffmpeg.EINVAL);
        }
    }
}
