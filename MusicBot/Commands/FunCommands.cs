...
public class FunCommands : ApplicationCommandModule<ApplicationCommandContext>
{
    [SlashCommand("snap", "Administrative command only.")]
    public async Task SnapAsync(
            [SlashCommandParameter(Name = "channel", Description = "Target Channel ID.")] string channelId)
    // remove everyone from the VC you are in
    {
        ...
        foreach (var user in usersInVc)
        {
            try
            {
                ...
                await userObject.ModifyAsync(act =>
                {
                    act.ChannelId = 0;
                });
            }
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"Failed to remove user {user.Id} from voice channel: {ex.Message}");
            }
        }
        
        await ModifyResponseAsync(message => 
            message.Content = $"Removed {usersInVc.Length} users from the voice channel. If nobody was removed, I do not have permission to do so.");
    }
}